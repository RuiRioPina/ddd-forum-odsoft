    ...
    return (
      <Layout>
        ...
        {this.getPostsFromActiveFilterGroup().map((p, i) => {
          /**
           * Made changes here to achieve the goal of this exercise: to highlight today's posts with another color.
           * Used DateUtil class to check where the date in the post's createdAt property is today.
           *
           * Results:
           * If it is change the background color of the post to red.
           * If it isn't leave the background color as it is.
           */

          const isToday = DateUtil.isToday(new Date(p.createdAt)); // Use DateUtil.isToday to check if the post is from today

          // Define a style object with red background for today's posts
          const postStyle = isToday ? { backgroundColor: 'red' } : {};
